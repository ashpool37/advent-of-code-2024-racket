...
(define (make-trail-finder topography)
  (define/memoize (evaluate-fn trail-start-pos)
    (cond
      [(= 9 (get-height topography trail-start-pos)) (list trail-start-pos)]
      [else
       (for/fold
           ([reachable-trail-ends (list)])
           ([dir dirs-all])
         (define trail-continuation (vec2d+ trail-start-pos dir))
         (set-union
          reachable-trail-ends
          (cond [(not (in-bounds? topography trail-continuation)) (list)]
...
(define (main)
  (let* ([topography (port->topography)]
         [get-trails (make-trail-finder topography)])
    (seq:foldl
     + 0
     (seq:map (compose length get-trails)
